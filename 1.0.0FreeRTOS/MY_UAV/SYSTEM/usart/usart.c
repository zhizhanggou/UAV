...
u8 gpsRxBuffer[GPS_RX_BUFFER_SIZE];
...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
   ...
   if(UART2_Handler.Instance==USART2)//如果是串口1，进行串口1 MSP初始化
	{
		__HAL_RCC_GPIOA_CLK_ENABLE();			//使能GPIOA时钟
...
{	
 ...
 // HAL_UART2_MspInit();
	...
	HAL_UART_Receive_IT(&UART2_Handler, (u8 *)gpsRxBuffer, GPS_RX_BUFFER_SIZE);//该函数会开启接收中断：标志位UART_IT_RXNE，并且设置接收缓冲以及接收缓冲接收最大数据量
}




uint8_t gpsRecvEndFlag=0;
int16_t rx_len;
  ...
  UNUSED(huart);
	if(huart->Instance==USART2)//如果是串口1
	{
		if((USART_RX_STA&0x8000)==0)//接收未完成
		{
			if(USART_RX_STA&0x4000)//接收到了0x0d
			{
				if(gpsRxBuffer[0]!=0x0a)USART_RX_STA=0;//接收错误,重新开始
				else USART_RX_STA|=0x8000;	//接收完成了 
			}
			else //还没收到0X0D
			{	
				if(gpsRxBuffer[0]==0x0d)USART_RX_STA|=0x4000;
				else
				{
					USART_RX_BUF[USART_RX_STA&0X3FFF]=gpsRxBuffer[0] ;
					USART_RX_STA++;
					if(USART_RX_STA>(USART_REC_LEN-1))USART_RX_STA=0;//接收数据错误,重新开始接收	  
				}		 
			}
...
//#endif
	...
	timeout=0;
	while(HAL_UART_Receive_IT(&UART2_Handler, (u8 *)gpsRxBuffer, GPS_RX_BUFFER_SIZE) != HAL_OK)//一次处理完成之后，重新开启中断并设置RxXferCount为1
	{
	 timeout++; //超时处理
	 if(timeout>HAL_MAX_DELAY) break;	
	}
